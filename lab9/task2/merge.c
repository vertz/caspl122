...
int resolve_all_symbols(void *start1, void * start2) {
	...
	Elf32_Sym  *symbolHeader1=0;
	Elf32_Sym  *symbolHeader2=0;
	char* sym_table1=0;
	char* sym_table2=0;

	char* symName1,*symName2;
	char* secName;
	char* name;
	int i,j,k;
	int symSize1=0,symSize2=0;
	...
	for (i=0; i < header1->e_shnum ;i++){
		...
		if (strcmp(name,".symtab")==0){
			symbolHeader1=(Elf32_Sym *)(start1+sectionHeader1->sh_offset);
			symSize1=(int)(sectionHeader1->sh_size/sectionHeader1->sh_entsize);
		}
		if(strcmp(name,".strtab")==0){
			sym_table1=(char *)(start1 + sectionHeader1->sh_offset);
		}
		sectionHeader1++;
	}
	sectionHeader1=sectionHeader1-i;

	for (i=0; i < header2->e_shnum ;i++){
		name=string_table2+sectionHeader2->sh_name;
		if (strcmp(name,".symtab")==0){
			symbolHeader2=(Elf32_Sym *)(start2+sectionHeader2->sh_offset);
			symSize2=(int)(sectionHeader2->sh_size/sectionHeader2->sh_entsize);
		}
		if(strcmp(name,".strtab")==0){
			sym_table2=(char *)(start2 + sectionHeader2->sh_offset);
		}
		sectionHeader2++;
	}
	sectionHeader2=sectionHeader2-i;
	...
	if(DEBUG>0){
		printf("%-23s %-23s %-6s %-6s\n","Name","Section","Index2","Index1");
	}
	for (i=0; i < symSize1 ;i++){
		if (symbolHeader1->st_shndx == 0 || symbolHeader1->st_shndx >= header1->e_shnum){
		    symName1=sym_table1+symbolHeader1->st_name;
		    if(strcmp( symName1, "") != 0){
			for (j=0; j < symSize2 ;j++){
				symName2=sym_table2+symbolHeader2->st_name;
				if (strcmp(symName1,symName2)==0){
				    if (symbolHeader2->st_shndx != 0 && symbolHeader1->st_shndx < header2->e_shnum){
					if(DEBUG>0){
						secName=string_table2 + (sectionHeader2 + symbolHeader2->st_shndx)->sh_name;
						printf("%-23s %-23s %-6d ",symName1,secName,symbolHeader2->st_shndx);
						for (k=0; k < header1->e_shnum ;k++){
							name=string_table1+sectionHeader1->sh_name;
							if (strcmp(name,secName)==0){
								printf("%-6d\n",k);
								symbolHeader1->st_shndx=k;
								symbolHeader1->st_value=sectionHeader1->sh_size+symbolHeader2->st_value;
								break;
							}
							sectionHeader1++;
						}
						if(k==header1->e_shnum){
							printf("\n");
							printf("error couldnt find the section in file1\n");
							return -1;
						}
						sectionHeader1=sectionHeader1-k;
			    		}
					break;
				    }
				    else{
